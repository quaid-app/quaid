...
- [x] 14.1 Extend the existing DAB harness to add a §8 Conversation Memory section
- [x] 14.2 Wire the LoCoMo adapter (or its existing harness equivalent) into §8: load fixture sessions, ingest via `memory_add_turn`, close sessions, run the LoCoMo test queries against the resulting fact set, score
- [x] 14.3 Repeat for LongMemEval as a parallel sub-section
  > **Closure note (Fry, 2026-05-05):** The landed §8 surface is `benchmarks/dab_section8.py` plus new `--mode conversation-memory` paths in `locomo_eval.py` and `longmemeval_adapter.py`. LongMemEval uses a truthful fact-page proxy metric (`answer_hit_at_5`) because the dataset's session-level evidence ids do not align with extracted fact-page slugs.
- [x] 14.4 Set the regression gate: §8 Conversation Memory must not regress more than 3 absolute percentage points version-over-version
  > **Closure note (Fry, 2026-05-05):** "3 points" here means 3 absolute percentage points on the subsection score. The baseline anchor is `benchmarks/baselines/conversation_memory.json`; until a full representative-hardware run records scores there, the gate reports `pending-baseline` rather than pass/fail.
- [x] 14.5 Add the truthful workflow hook the repo can support today: a manual hosted-runner smoke run plus documented full representative-hardware execution
  > **Closure note (Fry, 2026-05-05):** The truthful landed surface is `.github/workflows/conversation-memory-benchmarks.yml`, a manual GitHub-hosted smoke hook, plus documentation that the authoritative gate still requires a full representative Unix hardware run with no `--limit`. This repo does **not** yet claim an always-on representative-hardware CI gate.

## 15. Integration tests + benchmarks

> **Closure note (Scruffy, 2026-05-05T17:17:29.932+08:00):** This batch truthfully closes 15.1, 15.2, and 15.4 with real integration coverage. `15.1` uses the repo's existing online-model/mock-HF seam plus a request-counter proof that the post-`extraction enable` turn/close/extraction path stays offline; on this Windows lane we do **not** claim Linux network-namespace isolation. `15.4` drives the real turn-capture + close + worker + fact-file + ingest + `memory_search` path; the watcher hop is represented by the existing single-file ingest seam because the repo does not run a background watcher inside integration tests.

- [x] 15.1 `tests/airgap_extraction.rs`: run extraction enable + a turn add + extraction through the repo's mock-HF/local-cache seam; assert zero outbound model-network calls after enable completes
- [x] 15.2 `tests/extraction_idempotency.rs`: extract a session, then `--force` re-extract; verify the resulting head set is structurally equivalent (same `(kind, type_key)` partitioning, same chain shape)
- [x] 15.3 `benches/extraction.rs`: per-window p95 latency under representative load — assert `< 3s` on M1/M2 Mac fixtures and `< 8s` on x86_64 Linux fixtures
  > **Closure note (Copilot, 2026-05-05):** The repo now ships `cargo bench --bench extraction`, a manual representative-hardware bench that warms the local cached model, measures real per-window extraction latency, enforces the threshold on supported fixtures (`macOS/aarch64`, `linux/x86_64`), and reports `SKIPPED` elsewhere unless explicitly forced.
- [x] 15.4 End-to-end smoke test: capture a 50-turn session via `memory_add_turn`, close it, wait for extraction, assert at least one fact page exists in `<vault>/extracted/`, assert `memory_search` over the conversation topic returns the fact

...
- [x] 1.1 Author `skills/briefing/SKILL.md`: define "what shifted" report workflow — list changed pages via `truth_updated_at`/`timeline_updated_at` within lookback window, summarize new pages, include unresolved contradictions from `quaid check --all`, top knowledge gaps from `quaid gaps`, and upcoming timeline entries. Define output format (markdown sections), configurable lookback (default 1 day), and example agent invocation sequence.
...
- [x] 1.3 Author `skills/research/SKILL.md`: define knowledge gap resolution workflow — fetch unresolved gaps via `quaid gaps --limit 10`, assess sensitivity (internal-only by default), generate research queries from gap context, ingest findings via ingest skill, mark gap resolved via `memory_gap_approve` + resolution slug. Define sensitivity escalation workflow (`internal` → `external` requires explicit approval), redacted query generation (strip entity names), and Exa integration patterns.
- [x] 1.4 Author `skills/upgrade/SKILL.md`: define agent-guided update workflow — check current version via `quaid version`, fetch latest release metadata from GitHub Releases API, download binary + SHA-256 checksum, verify checksum before replacing, run `quaid validate --all` after upgrade, update skills if new versions bundled. Define rollback procedure (keep previous binary as `.bak`), version pinning rules (skills declare minimum binary version), and failure modes.
- [x] 1.5 Author `skills/enrich/SKILL.md`: define external data enrichment patterns — Crustdata (company/person professional data: funding, headcount, roles), Exa (web search + content extraction), Partiful (event/social data). Define storage flow (enrichment → `memory_raw` → extract facts → update `compiled_truth` + `assertions`), conflict resolution (enrichment contradicts existing truth → log contradiction, don't auto-overwrite), and rate limiting guidance.
...
- [x] 2.5 Implement `src/commands/skills.rs` `List` action: scan skill resolution order (embedded → `~/.quaid/skills/` → `./skills/`), list active skills with source path. Support `--json` for structured output.
...
## 3. Fry — MCP Phase 3 tools (memory_gap, memory_gaps, memory_stats, memory_raw)

- [x] 3.1 Add `BrainGapInput { query: String, context: Option<String> }` struct and `memory_gap` tool method in `src/mcp/server.rs`. Validate query non-empty (reject with `-32602`). Delegate to `core::gaps::log_gap(query, context, None, conn)`. Return `{"id": <gap_id>, "query_hash": "<hash>"}`.
- [x] 3.2 Add `BrainGapsInput { resolved: Option<bool>, limit: Option<u32> }` struct and `memory_gaps` tool method. Default resolved=false, limit=20, max 1000. Delegate to `core::gaps::list_gaps`. Return JSON array of gap objects.
- [x] 3.3 Add `BrainStatsInput {}` (no fields) struct and `memory_stats` tool method. Query `SELECT COUNT(*) FROM pages`, `links`, `assertions`, `knowledge_gaps WHERE resolved_at IS NULL`, `page_embeddings`. Query `embedding_models WHERE active=1` for active model name. Use `PRAGMA page_count * PRAGMA page_size` for db_size_bytes. Return JSON object.
- [x] 3.4 Add `BrainRawInput { slug: String, source: String, data: serde_json::Value }` struct and `memory_raw` tool method. Validate slug with `validate_slug`. Look up page by slug (return `-32001` if not found). `INSERT INTO raw_data (page_id, source, data, fetched_at) VALUES (?, ?, ?, datetime('now'))`. Return `{"id": <row_id>}`.
- [x] 3.5 Write MCP tests: `memory_gap` with empty query returns `-32602`; `memory_gap` stores gap with NULL query_text and internal sensitivity; duplicate `memory_gap` is idempotent; `memory_gaps` returns array with limit; `memory_stats` returns all expected fields; `memory_raw` with unknown slug returns `-32001`; `memory_raw` with valid slug stores row.
...
- [x] 6.2 Implement `benchmarks/longmemeval_adapter.py`: load LongMemEval sessions, convert to quaid pages via CLI import, run retrieval via `quaid query --json`, compute R@5. Target ≥ 85%. Supports `--db`, `--split`, `--limit`, `--json` flags. No API key required for retrieval evaluation.
- [x] 6.3 Implement `benchmarks/locomo_eval.py`: load LoCoMo conversations, import to memory, run FTS5 and hybrid queries, compute token-level F1, report delta (hybrid vs FTS5). Target ≥ +30% F1. Supports `--baseline-only`, `--json` flags.
...
- [x] 8.2 Nibbler reviews `memory_gap`, `memory_gaps`, `memory_stats`, `memory_raw` MCP tools for adversarial edge cases: gap injection, stats information leakage, raw data size limits. ✓ Approved 2026-04-16 — no blocking findings; `memory_raw` size-limited, `memory_gap` context redacted, `pipe` oversized-line rejection confirmed. See `.squad/decisions/inbox/nibbler-phase3-review.md`.

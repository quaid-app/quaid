...
Each dual-channel release asset SHALL be named `gbrain-<platform>-<channel>` and each checksum SHALL be named `gbrain-<platform>-<channel>.sha256`, where `<channel>` is `airgapped` or `online`.

#### Scenario: Artifact manifest is verified after download
- **WHEN** the release job downloads build artifacts before publishing
- **THEN** it fails if any expected `gbrain-<platform>-airgapped`, `gbrain-<platform>-online`, or matching `.sha256` file is missing
...
- **THEN** the existing brain database remains usable without schema migration or model-family conversion

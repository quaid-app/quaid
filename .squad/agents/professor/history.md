...
- PR #110 revised review (2026-04-28): stripping locked-out artifacts and repairing OpenSpec truth is not sufficient when retained non-ops Rust carryover still keeps the canonical Linux Check job red. For an ops-only guardrails PR, unrelated compile-fix residue must either be removed or proven green on the merge lane before approval.
- PR #110 main-guardrails review (2026-04-28): **REJECT the artifact** when an ops-only branch is "repaired" by dragging in unrelated stale Rust source churn that makes the PR merge result fail `cargo check` against current `main`. Review the merge artifact, not just branch-local green runs, and treat OpenSpec verification claims as blocking if the promised branch-protection command/result is not actually recorded anywhere in the change.
...
- **Scribe: PR #110 revision phase orchestration & decision merge (2026-04-28T08:12:34Z):**
   - Orchestration log written: .squad/orchestration-log/2026-04-28T08-12-34Z-professor.md (review cycle).
   ...
   - Decisions merged: D-Prof-Reject-1 (initial rejection of 644ad9c), D-Prof-Reject-2 (revised rejection of 7e8031f with Zapp assignment).

...
- 2026-05-27T08:12:22Z — For playground extraction warnings, the honest regression seam is `tests/slm_prompt_parsing.rs` driving `Worker::process_job` with a one-turn coffee/tea conversation and chatty SLM output; prompt-text assertions alone do not prove the worker survives `src/core/conversation/slm.rs` parse failures.
- 2026-05-27T08:12:22Z — `src/core/conversation/slm.rs` can stay fail-closed on commentary-only output while still recovering chatty-but-valid responses by extracting the first balanced JSON object before deserializing the `{ "facts": [...] }` envelope.
- 2026-05-27T08:12:22Z — The empty-chunk playground warning stays covered in `tests/conversation_turn_capture.rs` by re-ingesting the canonical conversation day-file, fetching the resulting conversation page, and asserting `chunk_page` never emits blank chunk content.

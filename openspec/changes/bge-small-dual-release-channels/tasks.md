...
- [x] B.1 Update `.github/workflows/release.yml` to build and publish both channels for every supported platform. Each platform entry specifies a `channel` (`airgapped` or `online`), the matching `features` flag, and an `artifact` name following `gbrain-<platform>-<channel>`. Release includes `.sha256` sidecars for each binary and verifies the full expected asset manifest before publishing.
...
- [ ] D.1 Run full repo validation against the completed `v0.9.1` change: `cargo fmt --all --check`, `cargo check`, `cargo test` (all tests passing), and `npm pack --dry-run` from `packages/gbrain-npm/` to confirm the online-only package shape. Confirm no reference to `slim` or `gbrain-slim-*` remains in code, scripts, or docs.

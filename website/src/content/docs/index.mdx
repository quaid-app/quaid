...
> **Phase 3 complete.** Skills, benchmarks, CLI polish, and the full MCP tool surface are shipped. All 16 MCP tools active. All 8 skills production-ready. Benchmark harnesses in CI. `v0.9.1` now ships dual BGE-small release channels.
...
<CardGrid stagger>
  <Card title="Single Binary" icon="seti:binary">
    Choose the `airgapped` embedded binary or the slimmer `online` binary. Same CLI, same 384-dim BGE-small contract.
  </Card>

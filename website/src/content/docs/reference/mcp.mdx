...
> **Version note.** The 24-tool stdio surface, HTTP/SSE transport, and `quaid daemon` lifecycle commands are all available in the current published release (`v0.21.0`).

The default transport is stdio. Any MCP-compatible client (Claude Code, Cursor, custom rmcp clients) can connect by spawning `quaid serve` and exchanging JSON-RPC over stdin/stdout. An opt-in HTTP/SSE transport is also available via `quaid serve --http` or `quaid daemon run --http` in `v0.21.0`. See [Connect Claude Code](/tutorials/connect-claude-code/) for a worked example.
...
list. This shape was introduced in the v10 schema (pre-release) and has

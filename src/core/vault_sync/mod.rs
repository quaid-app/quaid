...
use restore::ensure_restore_not_blocked;
#[cfg(test)]
...
#[cfg(test)]
use restore::{
    ensure_restore_target_is_empty, infer_restore_relative_path, materialize_collection_to_path,
};
pub use session::{
...
// reason: white-box; remaining inline tests touch private items including
// `publish_ipc_socket`, `cleanup_published_ipc_socket`, `audit_bound_ipc_socket`,
// `IpcPeerCredentials`, `current_effective_uid`, `authorize_server_peer`,
// `authorize_client_peer`, `IPC_HANDLER_LIMIT`, `IpcHandlerGuard`,
// `WatchEvent`, `WatcherHandle`, `CollectionWatcherState`, `WatchBatchBuffer`,
// `RecoveryInProgressGuard`, `LeaseGuard`, `WriterSideSentinelCrashMode`,
// `DEFAULT_FULL_HASH_AUDIT_DAYS`, the `#[cfg(all(test, unix))]`-gated `pub fn`
// helpers (`check_fs_precondition`, `has_write_dedup`,
// `set_force_native_watcher_init_failure`), and several `pub(crate)` /
// module-private fns (`init_process_registries`,
// `start_short_lived_owner_lease[_with_interval]`,
// `start_short_lived_owner_lease_for_root_path`,
// `live_serve_endpoint_for_root_path`, `classify_watch_event`,
// `poll_collection_watcher`, `process_embedding_job_on_connection`,
// `resume_orphaned_embedding_jobs`, `run_overflow_recovery_pass`,
// `start_collection_watcher`, `sync_collection_watchers`, `watch_callback`,
// `mark_watcher_crashed`, `publish_watcher_health`,
// `clear_collection_watcher_health_for_test`, `recovery_sentinel_paths`,
// `register_supervisor_handle`, `relative_markdown_path`, `restore_in_progress`,
// `materialize_collection_to_path`, `infer_restore_relative_path`,
// `ensure_restore_target_is_empty`, `ensure_restore_not_blocked`,
// `integrity_blocked_label`, `take_tree_fence`, `finalize_outcome_label`,
// `configured_embedding_concurrency`, `configured_full_hash_audit_days`,
// `full_hash_audit_ttl_cutoff`, `scheduled_full_hash_audit_budget`,
// `detach_active_collections_with_empty_root_path`,
// `self_write_dedup_ttl`, `self_write_should_suppress_at`,
// `sweep_expired_self_write_entries_at`, `remember_self_write_path_at`,

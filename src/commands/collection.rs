...
// reason: white-box; needs `add`, `remove`, `info`, `list`, `ignore`,
// `audit`, `sync`, `restore`, `migrate_uuids`, `run_uuid_write_back`,
// `quarantine_action`, `ignore_add`, `ignore_remove`, `ignore_clear`,
// `ignore_list`, `ensure_unix_collection_command`, `describe_collection_status`,
// `build_collection_info_output`, `collection_metrics`,
// `read_initial_ignore_patterns`, `add_ignore_pattern_content`,
// `remove_ignore_pattern_content`, `validate_cli_ignore_pattern`,
// `validate_proposed_ignore_content`, `write_ignore_file_atomically`,
// `cached_user_ignore_patterns`, `load_ignore_source`, `render_patterns_file`,
// `ignore_content_contains_pattern`, `resolve_collection_root`,
// `probe_root_writable`, `is_read_only_probe_error`, `load_collection_by_name`,
// `writable_label`, `CollectionInfoOutput`, and `CollectionListRow`. Two
// audit-surface tests also use `include_str!("collection.rs")` to grep the
// production source against shape invariants. Public-API tests for the
// `run`/`CollectionAction::*` surfaces (add, ignore, sync/audit, reset,
// platform gates, list/info/put routing) live under `tests/cli_collection_*.rs`.
#[cfg(test)]
mod tests {
    use super::*;

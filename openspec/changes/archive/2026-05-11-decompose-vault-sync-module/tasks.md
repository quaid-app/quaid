...
- [ ] 8.1 Run `find src/core/vault_sync -name '*.rs' -exec wc -l {} +`; confirm no file exceeds 800 LOC (PARTIAL — mod.rs has dropped from 7833 → 6019 LOC across this change; every other file under `src/core/vault_sync/` is under the 800 budget. The residual gap in mod.rs splits cleanly into two halves:

      **Production half (~3,100 LOC)**: ~1,500 LOC of legitimate orchestrators (`start_serve_runtime` + `run_supervisor_loop` + `bind_socket` + `spawn_watcher`, `list_memory_collections`, `finalize_pending_restore` + `finalize_pending_restore_via_cli` + `run_tx_b` + `run_rcrt_pass`, `start_short_lived_owner_lease[_with_interval]` + `LeaseGuard` + `ShortLivedLease`, `mark_collection_restoring_for_handshake` + `wait_for_exact_ack`, `start_collection_watcher` + `sync_collection_watchers` + `poll_collection_watcher` + `publish_watcher_health` + `run_overflow_recovery_pass` + `detach_active_collections_with_empty_root_path`, `write_quaid_id_to_file`, `ensure_plain_sync_allowed`, the per-collection `RuntimeRegistries` / `ServeRuntime` types, the constants block, the imports + `mod` decls + `pub use` block) plus ~600 LOC of supporting helpers that touch many of the above (`recover_owned_collection_sentinels`, `claim_owned_collections`, the `*supervisor_handle*` family, `complete_attach`, `convert_reconcile_error`, `with_self_write_dedup` + the rest of the self-write dedup family, `run_full_hash_audit_pass` + the audit budgeting helpers, `mark_collection_restoring_for_handshake` + `wait_for_exact_ack`, `revert_orphan_restore_state` + `revert_aborted_restore` + `clear_manifest_incomplete` + `pending_manifest_age_seconds` + `has_fresh_command_heartbeat`). A further sequel could pull `finalize_pending_restore` + its cluster into `restore.rs` (~450 LOC) and the `*supervisor_handle*` map into a new `vault_sync/supervisor.rs` (~150 LOC); after both, mod.rs's production half lands around ~2,500 LOC. Going substantially below that requires breaking up `start_serve_runtime` / `run_supervisor_loop` / `finalize_pending_restore_via_cli` themselves — those bodies are each under 200 lines and read better as one unit than as a graph of sub-helpers. The 800 LOC ceiling for mod.rs specifically is therefore not realistic without harming readability.

      **Inline tests (~2,900 LOC)**: 80+ `mod tests` cases live at the bottom of mod.rs because they exercise private items. Most of them now test items in submodules (embedding, ipc, restore, remap, watcher, recovery, write_lock) and could move to those submodules' own `mod tests` blocks once a shared `test_support` module exists; the per-feature integration-test split is already tracked by the sibling change `extract-inline-tests-to-integration`, which is the natural sequel for the test half.

      mod.rs progression across this change: 7833 → 7066 (§4.7/§4.8/§6 follow-ups) → 6806 (§4.7 begin_restore expansion) → 6201 (remap.rs extraction) → 6019 (embedding.rs + watcher leaf-bundle extraction). Across all 21 commits on this branch, ~1,800 LOC were removed from mod.rs. The 800 ceiling is closed-out as PARTIAL with the rationale above; further closure is sequel work.)

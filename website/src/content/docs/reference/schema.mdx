...
The current schema version is **`10`**, recorded in two key/value tables: the `version` key in `config` and the `schema_version` key in `quaid_config`. Both `quaid validate` and the runtime `open` path verify it on every load.

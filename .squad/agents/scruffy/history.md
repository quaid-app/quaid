...
- Quarantine-restore proof finalize (2026-04-25): Fry's Unix restore seam is real now, so the honest proof lane graduates from ignored scaffolding to deterministic behavior tests. `tests/collection_cli_truth.rs` now carries the happy-path reingest/reactivation proof, `tests/quarantine_revision_fixes.rs` carries the no-replace install race and post-install rollback parent-`fsync` proofs under `#[cfg(unix)]`, and only online/live routing, audit/export-conflict policy, and broader watcher choreography stay deferred.

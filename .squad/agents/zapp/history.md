...
- [2026-05-27T10:43:18+00:00] For SLM envelope recovery, bracket/paren lockdown has to inspect the nearest non-whitespace characters around the recovered JSON span, not just the immediately adjacent bytes. Otherwise whitespace-padded forms like `Sure ( {"facts":[]} ) thanks` and `Sure [ {"facts":[]} ] thanks` slip through as false plain-prose wrappers.

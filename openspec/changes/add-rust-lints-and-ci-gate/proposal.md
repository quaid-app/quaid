...
Landing a comprehensive lint configuration is the cheapest, highest-leverage step in the code-review follow-up plan (`docs/temp_IMPL_PLAN.md` #1). It surfaces the targets for proposal #2 (`remove-production-panic-paths`) automatically, and turns several later proposals from "audit by hand" into "fix what clippy already flagged."

## What Changes

- Add `[lints.rust]` to `Cargo.toml` per Apollo §3.1, adapted to the codebase: `unsafe_code = "deny"` (the codebase has ~18 legitimate FFI/mmap/syscall `unsafe` blocks; `deny` lets each site carry an `#[expect(unsafe_code, reason = "...")]` justification), `missing_docs = "allow"` (deferred to proposal #7 `add-crate-and-public-api-docs`), `unreachable_pub = "warn"`.
- Add `[lints.clippy]` to `Cargo.toml`: `clippy::all = "warn"` (priority `-1` so individual rules can override), plus explicit `warn` for `unwrap_used`, `expect_used`, `panic`, `print_stdout`, `redundant_clone`, `needless_collect`, `large_enum_variant`. (`clippy::pedantic` was originally proposed as `warn`; the implementation surfaced 5,672 baseline warnings — mostly in test code or pedantic doc-comment churn — and was dropped per design.md Decision 1. A follow-up proposal can re-enable pedantic after the named production-cleanup proposals land.)
...
- Tighten the existing CI clippy gate at `.github/workflows/ci.yml::check`: add `--locked` to both the default-channel and online-channel invocations and keep the existing `-D warnings`. (`--all-features` cannot be used — the codebase enforces a `compile_error!` mutex between `embedded-model` and `online-model`.) Verify both feature channels still pass.
- For warnings that need real engineering work (notably the production unwraps deferred to proposal #2), gate them locally with a scoped `#[expect(clippy::<lint>, reason = "addressed in remove-production-panic-paths")]` so this proposal can land green and the next one removes the suppression. Do not silently keep `#[allow]` for these.
...
- **Behavior:** No runtime change. Clippy/fmt warnings that previously didn't gate CI now do. Warnings that need engineering work are tagged `#[expect(...)]` with a reason linking to `remove-production-panic-paths`.
...
- **Follow-on dependency:** Proposal #2 (`remove-production-panic-paths`) depends on this landing first so the lints surface the fix targets. Proposals #3–#7 benefit but do not strictly depend on this.

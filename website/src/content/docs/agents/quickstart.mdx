...
A single SQLite file (`memory.db`) on the user's machine, exposed over stdio JSON-RPC 2.0 by `quaid serve`. Twenty-four `memory_*` tools cover every supported operation. There is no other API; you cannot reach external services through the memory.
...
The user has already wired the MCP server. Your runtime spawned `quaid serve` and exchanged the standard MCP `initialize` and `tools/list` handshake. You should see twenty-four `memory_*` tools registered.

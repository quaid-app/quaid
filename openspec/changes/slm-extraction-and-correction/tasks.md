...
- [ ] 14.4 Set the regression gate: §8 Conversation Memory must not regress more than 3 points version-over-version (matching §4's existing tolerance)
  > **Scope note (Bender, 2026-05-05):** "3 points" here means 3 absolute F1 percentage-point drop, **not** "matching §4's 2% nDCG@10 relative regression tolerance" — those are different metrics and different thresholds. The baseline anchor is `benchmarks/baselines/section8.json` (created; both baselines null/pending until a §8-compliant harness is run). The pre-existing `locomo_eval.py`/`longmemeval_adapter.py` adapters ingest raw conversation pages and do **not** satisfy §8; a new harness must drive `memory_add_turn` → session close → extraction worker → query fact pages before a numeric baseline can be established.
- [ ] 14.5 Add a CI run that exercises the full §8 path on representative hardware
  > **Scope note (Bender, 2026-05-05):** A full §8 pipeline CI run requires Phi-3.5 Mini (~2 GB). GitHub Actions ubuntu-latest cannot silently download this per-PR. The correct pattern is `#[ignore]`-guarded + manual gate on release branches, matching the existing BEIR `beir_eval.rs` gate (`cargo test --test <name> -- --ignored`). Do **not** wire a per-PR CI step that downloads the model; gate it to the `benchmarks` CI job which already follows the manual-only pattern.

## 15. Integration tests + benchmarks

> **Closure note (Scruffy, 2026-05-05T17:17:29.932+08:00):** This batch truthfully closes 15.1, 15.2, and 15.4 with real integration coverage. `15.1` uses the repo's existing online-model/mock-HF seam plus a request-counter proof that the post-`extraction enable` turn/close/extraction path stays offline; on this Windows lane we do **not** claim Linux network-namespace isolation. `15.4` drives the real turn-capture + close + worker + fact-file + ingest + `memory_search` path; the watcher hop is represented by the existing single-file ingest seam because the repo does not run a background watcher inside integration tests. `15.3` remains open because this repo's benchmark conventions are ignored integration gates under `tests/` plus the Python `benchmarks/` harness, not a Cargo `benches/` surface with hardware-specific latency assertions.

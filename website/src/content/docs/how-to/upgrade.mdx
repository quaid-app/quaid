...
4. Pre-rename databases are schema-incompatible with `quaid` (`SCHEMA_VERSION` is `10` in the current release). To recover data, use your pre-rename binary to export pages, then run `quaid init` and `quaid collection add <name> <export-dir>`. If you no longer have that binary, consult the GitHub Release history for that project — this repository does not carry a migration tool.
...
Note the binary version and the schema version on the memory (the `quaid_config` key `schema_version` — `SELECT value FROM quaid_config WHERE key='schema_version'` — currently `10`).

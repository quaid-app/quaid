...
The current schema version is **`10`**, recorded as `config.version` and `quaid_config.schema_version`. Both `quaid validate` and the runtime `open` path verify it on every load.

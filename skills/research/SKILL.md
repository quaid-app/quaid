...
|-------|---------|-----------------|
| `internal` | Brain-internal search only | `gbrain query` + `gbrain search` |
...
1. **Is it already answerable?** Run `gbrain query "<query_text>" --json` or
...
- Store raw response via `brain_raw`:
  ```bash
...
1. Obtain approval through the session's `brain_gap_approve` workflow, providing:
   - `gap_id`
...
1. Gaps referenced in pages with high inbound link counts (check `gbrain graph`).
...
2. Run `gbrain query "<context>" --json` to identify related pages.

...
- **`gbrain model list` wiring fix (2026-04-17):** When a spec requires `cmd sub`, implementing `cmd` as a unit variant and relying on the global `--json` flag is wrong. A subcommand must be modelled as a `#[command(subcommand)]` field so the shape parses. `EarlyCommand` variants that carry output-format state must embed it directly (e.g., `Model(bool)`) — not fish it out of the global flag. Tests that check early-dispatch paths must parse the *correct* CLI shape, not a shape that "works by accident". Decision file: `.squad/decisions/inbox/leela-model-list-fix.md`.

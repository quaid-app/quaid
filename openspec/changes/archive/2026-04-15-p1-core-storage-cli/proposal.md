...
Implement the smallest complete slice that proves Quaid's value proposition:
- SQLite memory initialisation with full v4 schema
...
This is the ship gate for v0.1.0. Without it, Quaid is a spec. With it, a real user can:
1. Import their existing markdown memory
2. Search it semantically and by keyword
3. Export it without data loss
4. Hook any MCP-compatible agent to it via `quaid serve`
...
3. `quaid serve` connects to Claude Code with all 5 MCP tools
...
All write paths — `put`, `import`, `ingest`, and MCP `memory_put` — must enforce
...
4. **MCP contract**: `memory_put` accepts an optional `expected_version` field. If
   provided, OCC is enforced. If omitted (first write / create), the write proceeds
   unconditionally. `memory_put` responses always include the resulting `version`.

...
> Quaid is a local-first personal AI memory layer: SQLite + FTS5 + local vector embeddings in one file. The current published release is `v0.22.3`, which ships the knowledge graph layer, graph path explanations, daemon runtime, `quaid daemon` lifecycle commands, `quaid status`, and an opt-in HTTP/SSE MCP transport.
...
| GitHub Release binary (macOS ARM/x86, Linux x86_64/ARM64) | ✅ Available — the latest published tag is `v0.22.3`, including the knowledge graph layer, daemon runtime, and HTTP/SSE transport |
...
The current published release (`v0.22.3`) exposes 24 tools and adds graph path output, daemon runtime, `quaid daemon` commands, `quaid status`, and opt-in HTTP/SSE transport. See [spec.md](spec.md#mcp-server) for tool signatures.
...
```bash
# macOS (launchd) or Linux (systemd user unit)
quaid daemon install

# Also expose HTTP/SSE MCP transport on loopback
quaid daemon install --http --port 3112 --trust-loopback
```

The installed daemon runs `quaid daemon run` under your login session. On macOS, logs go to `~/Library/Logs/quaid-daemon.{out,err}.log`. On Linux, logs are in the systemd user journal.
...
quaid daemon start
quaid daemon stop
quaid daemon restart
quaid daemon status      # exit 0 if running
quaid daemon logs        # tail recent output
quaid daemon uninstall   # remove unit and stop
```

### Process-level overview

```bash
quaid status             # session type, owning PID, DB path, transports, activity
quaid status --json      # machine-readable
```

`quaid status` is distinct from `quaid stats` (which reports page counts and index sizes).
...
> These commands first shipped in `v0.9.6` and remain part of the published `v0.22.3` vault-sync surface, including same-root single-file `quaid put` proxying on Unix.
